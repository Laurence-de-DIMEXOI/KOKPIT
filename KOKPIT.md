...
- **SAV** (`/commercial/sav`) — sync depuis Sellsy via custom field "Etat des produit = SAV", endpoint `GET /orders/{id}/custom-fields` (le `embed=customfields` ne marche PAS sur `/orders/search`). Types commentaires : NOTE / APPEL / MESSAGE / MAIL / COURRIER.
  - **Documents** : upload de vrais fichiers via `/api/upload` (Supabase, bucket `kokpit-media`, max 15 Mo) — bouton « Téléverser » dans le drawer (auto-remplit nom/type/taille), + option lien externe. (Avant : lien/notes uniquement → impossible de joindre un PDF.)
  - **Liens BCDI auto + remboursement (juin 2026)** : les dossiers SAV manuels ont un champ `sellsyBdcRef` en texte libre (Michelle saisit des formats variés : `BCDI-05066 ET BCDI-05452`, `BCDEI-05109` typo, `BCDI-05083 / BC 11680`…). Résolution **à la volée** (pas de stockage) via `GET /api/sav/[id]/sellsy` : `parseBcdiRefs()` (`lib/sav-bcdi.ts`, tolère typo/séparateurs) extrait tous les BCDI → recherche chaque commande Sellsy (`/orders/search` par numéro) → affiche statut + lien dans le drawer.
    - **Remboursement** = un **avoir** (note de crédit). En V2 l'avoir n'est rattaché qu'au **CLIENT** (`related` = individual/contact), pas au BCDI. On scanne donc tous les avoirs (`/credit-notes/search`, ~162, cache mémoire 10 min — filtres Sellsy non fiables) et on pré-filtre par id client (collectés sur les commandes résolues + `sellsyContactId`). `status` : `spent` = soldé / `stored` = émis non soldé.
    - **Lien précis avoir ↔ BCDI via la chaîne documentaire (V1)** : on remonte `Document.getOne` par `parentid` depuis l'avoir : `avoir → FAPJ → [BDL] → BCDI` (doctype biaisé par préfixe d'ident pour limiter les appels ; cache mémoire `docNodeCache`). Si le BCDI racine ∈ BCDI du SAV → `matchesSav=true` (badge « ✓ lié à BCDI-xxxx »). Validé : AVR-2026-0172 → FAPJ-12240 → **BCDI-05066** ; AVR-2026-0167 → FAPJ-12092 → BDL-2026-4801 → **BCDI-05083**.
    - **Semi-auto (choix Laurence)** : bannière « Remboursement détecté — AVR-xxxx, soldé/non soldé (+ ✓ lié à BCDI-xxxx) » + bouton **« Marquer ce SAV traité »** (1 clic → TRAITE). Pas de bascule auto (humain confirme). Si l'avoir n'est relié qu'au client (pas de BCDI commun), message « à vérifier ».
...
- **Prévisionnel achat** (`/achat/previsionnel`) — projection trésorerie + potentiel commercial par arrivage. Onglets par IMP (cf. `imports-config.ts`). Une ligne par BCDI (groupé sur le packing list) avec colonnes : BCDI · Client · Propriétaire · Statut · Nb meubles · Total HT · Reste à payer HT · Potentiel comm. Drawer expandable (ChevronRight → meubles + qty + note ; pour STOCK : prix HT unitaire + sous-total).
  - **Architecture lecture pure DB** : la page ne fait **aucun appel Sellsy live**. Toutes les valeurs (client, commercial, totalHT, restePayerHT, status, etatProduit, isSav) sont lues depuis la table `bcdi_sellsy_snapshot`. Prix HT catalogue STOCK : batch DB unique (SellsyDeclinationCache + SellsyItemCache). Cache mémoire 5 min pour la cohérence dans la session. Latence cible : **50 ms en chaud**, ≈ 1 s en cold start Vercel.
  - **Refresh par cron** : `/api/cron/previsionnel-refresh` (cron Vercel `*/30 * * * *`), Bearer `CRON_API_SECRET` ou UA `vercel-cron`. Pour chaque IMP, charge le packing JSON, identifie les BCDIs et appelle `refreshBcdiSnapshots()` (`src/lib/previsionnel-fetch.ts`) qui ne rafraîchit que les snapshots > 30 min. Param `?force=true` pour tout refetch immédiatement. Param `?imp=IMP-XXX` pour scoper.
  ...
  - **Badge SAV/RETOUR** : si le custom field Sellsy « Etat des produits » du BDC = `SAV` ou `RETOUR`, badge violet affiché à côté du statut + reste à payer remis à 0 (n'entre pas dans la projection trésorerie). Cache local Vente puis fallback live Sellsy `/orders/{id}/custom-fields`. ⚠ L'override `isSav` (reste à 0) est appliqué **après** la résolution BO → un BC BO repassé en SAV est bien remis à 0. Si une ligne `Vente` existe avec un `etatProduit` périmé, elle a priorité sur le live → la mettre à jour (le cron `refresh-etat-produit` nocturne le fait, ou maj directe).
  - **Saisie manuelle du reste à payer SUPPRIMÉE** (juin 2026) : le reste à payer est désormais réel partout (DIMEXOI = paiements Sellsy ; BO = paiements Sellsy BO). Action `set-amount` retirée de l'UI et de la route override ; champs `restePayerHTOverride`/`totalHTOverride` de `previsionnel_bcdi_override` conservés en base mais nettoyés (plus alimentés). Les **notes** de suivi client restent.
  - Endpoints : `GET /api/achat/previsionnel?imp=IMP-XXX[&fresh=true]` (cache 15 min), `POST /api/achat/previsionnel/override { bcdi, impCode, action: "to-stock" | "restore" | "set-note", note? }`.
  - **Mode prévisionnel + volume (IMP-619, juin 2026)** : le packing JSON accepte 4 champs optionnels par item — `section` ("command" | "stock" | "bonus", défaut "command"), `urgent` (bool, lignes rouges du fichier Indonésie = ultra-urgent à ne pas négocier), `volumeM3` (volume estimé/unité), `order` (préserve l'ordre de priorité du fichier). Groupement par clé composite `section::bcdi` (un même BCDI peut exister en commande ET en stock). Sections stock/bonus traitées comme stock potentiel quel que soit le préfixe BCDI. Colonne **Volume** triable + total tfoot + **jauge de remplissage container** (segmentée commande/stock/bonus vs `capacityM3`). Config arrivage : champs optionnels `containerType` (ex "40ft"), `capacityM3` (40ft std = 67.7, 40ft HC = 76.4), `previsionnel`. Filtres UI : catégorie + ultra-urgent + séparateurs de section en vue par défaut. Réfs produit vides = laissées vides (les BCDI existent et seront liés par le cron snapshot — pas de badge). Estimation volume : meubles assemblés = encombrement L×l×H ; lits = somme des panneaux à plat (tête + pied à la largeur matelas, pans de lit à la longueur — 2 par défaut, 3 pour certains modèles type "Ben canned" —, botte de lattes à la largeur). Jauge : segments en `inset-0` + `shrink-0` (sinon les % se calculaient sur une largeur nulle → barre vide) ; dépassement >100% coupé par `overflow-hidden`.
  - **Reste à payer via Bois d'Orient — "BO" (juin 2026)** : certains BCDI DIMEXOI sont en réalité des BC Bois d'Orient → leur total HT côté Sellsy DIMEXOI est à 0€. Le n° de BC figure rarement dans l'**objet** (subject V2), et le plus souvent dans un **commentaire** Sellsy (`"BCDI Bois D'Orient : BC-11487-06122024"`).
    - ⚠ **Les filtres de `POST /comments/search` sont 100% inopérants sur ce compte** (même `owner_id` renvoie les ~7157 commentaires ; seul un nom de filtre invalide renvoie 400). Impossible de cibler les commentaires d'une commande côté serveur.
    - `previsionnel-fetch` : si totalHT ≤ 0 → `extractBcNumber()` sur l'objet, sinon `getOrderBcMap()` qui **scanne tous les commentaires une fois** (paginé, plafonné 150 pages, mémoïsé 6h) et construit `orderId V2 → n° BC` (les commentaires portent `related:[{type:"order", id}]` dans l'espace d'id V2). Le `bdoBcNumber` est persisté dans le snapshot → `priorBcByBcdi` évite de re-scanner les BCDI déjà mappés aux refresh suivants.
    - **Montants depuis la base locale** `DocumentBoisDOrient` (`lookupBoAmountsLocal()`). Match par cœur `BC-<digits>` (référence en base = `BC-11487-06122024`). Total HT = montant HT de la commande BO. **Rattrapage** : si la commande est en base avec un montant à 0 (import BO incomplet), on récupère les montants en live via `fetchBoOrderAmounts()` (`GET /orders/{id}?field[]=amounts`) et on met la base à jour. Si le BC est réellement vide côté Sellsy BO (total 0 à la source), on conserve quand même client + référence (total 0 = BC non encore chiffré côté BO, pas un bug).
    - **Reste à payer HT exact (acomptes inclus)** : la table locale n'a pas le détail des paiements → on récupère la **Σ des paiements confirmés TTC** via `GET /orders/{id}/payments` sur le **Sellsy BO** (`fetchBoOrderPaidTTC()` dans `sellsy-bdo.ts`), mise en cache dans `DocumentBoisDOrient.paidInclTax` / `paidSyncedAt` (TTL 6h, migration `add_paid_incl_tax_to_doc_bois_dorient`). reste HT = (montantTTC − paidTTC) × (montantHT/montantTTC). Filet si fetch KO : commande `paid` ou facture BO payée du même montant → 0, sinon total. (C'est le **seul** appel Sellsy BO résiduel, mis en cache ; BO sera bientôt abandonné.)
    - Snapshot : champ `bdoBcNumber` (migration `add_bdo_bc_number_to_bcdi_snapshot`). Badge indigo **"BO BC-xxxxx"** sur la ligne ; les montants BO remplacent le 0€. Validé : BCDI-04687 → BC-11487 = 4 553 € HT, reste 0 € ; BCDI-05232 → BC-11727 = 7 834 € HT, 3 400 € TTC payés → reste ≈ 4 700 € HT.
  - **État validé (19 juin 2026) — page considérée finale par Laurence.** Sur IMP-619, plus aucun 0€ « anormal ». Les 0€ restants sont tous légitimes : stock magasin « ORDER DIMEXOI » (auto-stock → potentiel, pas de reste), lignes **SAV** (reclassées via le custom field Sellsy → reste 0), ou erreur de saisie BO (ex: BCDI-05550 Tiny House, pas de vrai BC). Refresh forcé de tous les IMP effectué (`refreshBcdiSnapshots` forceAll) + sync catalogue lancée → prix stock à jour (ex: EFWS 486R = 1 390,78 € HT).
...
| `/api/sellsy/sync-catalogue` | `0 4 * * *` | tous les jours 8h | Items + déclinaisons (quotidien depuis juin 2026 → prix stock du Prévisionnel à jour plus vite ; oldest-first, ~300 parents/run) |
