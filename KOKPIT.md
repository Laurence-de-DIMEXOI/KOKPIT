...
- **Prévisionnel achat** (`/achat/previsionnel`) — projection trésorerie + potentiel commercial par arrivage. Onglets par IMP (cf. `imports-config.ts`). Une ligne par BCDI (groupé sur le packing list) avec colonnes : BCDI · Client · Propriétaire · Statut · Nb meubles · Total HT · Reste à payer HT · Potentiel comm. Drawer expandable (ChevronRight → meubles + qty + note ; pour STOCK : prix HT unitaire + sous-total).
  - **Reste à payer HT** : source de vérité = **V1 Sellsy `Document.getOne(order)`** → champ `relateds_amount` qui agrège paiements directs + factures d'acompte liées (c'est ce que Sellsy utilise pour calculer le « Total à régler » du PDF du BDC). Formule : `reste TTC = totalAmount V1 − relateds_amount`, converti en HT via le ratio HT/TTC. Cas particuliers : `paid` → 0 ; `cancelled`/`refused`/`expired` → 0. **Fallback V2** : si l'appel V1 échoue (rare), retombée sur status + deposit + paiements directs `/orders/{id}/payments`. ⚠️ Limite : ne capte pas la chaîne BCDI → BDL → FAPJ finale dans certains cas (paiements migrés vers la facture finale qui n'apparaissent pas dans `relateds_amount` côté BDC) — écart -10 % sur IMP 618 (75k vs 83k Michelle). Tous les filtres Sellsy V1+V2 sur `parentid`, `linkedid`, `contact_id`, `individual_id` sont cassés/ignorés (testés exhaustivement), donc impossible de filtrer la recherche par client ou parent : pour aller plus loin il faudrait scanner toutes les factures et BDL puis croiser côté code (envisagé en cron nuit / webhook Sellsy).
