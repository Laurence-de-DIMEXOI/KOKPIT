...
- **Prévisionnel achat** (`/achat/previsionnel`) — projection trésorerie + potentiel commercial par arrivage. Onglets par IMP (cf. `imports-config.ts`). Une ligne par BCDI (groupé sur le packing list) avec colonnes : BCDI · Client · Propriétaire · Statut · Nb meubles · Total HT · Reste à payer HT · Potentiel comm. Drawer expandable (ChevronRight → meubles + qty + note ; pour STOCK : prix HT unitaire + sous-total).
  ...
  - **Reste à payer via Bois d'Orient (juin 2026)** : certains BCDI DIMEXOI sont en réalité des BC Bois d'Orient → leur total HT côté Sellsy DIMEXOI est à 0€, et le n° de BC figure dans l'**objet du document** (subject V2), parfois dans un **commentaire** (`"BCDI Bois D'Orient : BC-11487-06122024"`). `previsionnel-fetch` : si totalHT ≤ 0, `extractBcNumber()` cherche `BC[\s-]?\d{4,6}` dans l'objet puis, à défaut, dans le payload V1 `Document.getOne` complet (objet + notes + commentaires). Le BC trouvé est cherché dans le **Sellsy Bois d'Orient** via `fetchBdoOrderSummaryByNumber()` (`sellsy-bdo.ts`, plusieurs formes de numéro testées : `BC-11487` / `BC 11487` / `BC11487` / `11487`) qui calcule total HT + reste à payer HT (statut + paiements confirmés, même procédé que DIMEXOI). Snapshot : champ `bdoBcNumber` (migration `add_bdo_bc_number_to_bcdi_snapshot`). Badge indigo **"BDO BC-xxxxx"** sur la ligne ; les montants BDO remplacent le 0€.
