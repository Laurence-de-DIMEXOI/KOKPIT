...
- **SAV** (`/commercial/sav`) — sync depuis Sellsy via custom field "Etat des produit = SAV", endpoint `GET /orders/{id}/custom-fields` (le `embed=customfields` ne marche PAS sur `/orders/search`). Types commentaires : NOTE / APPEL / MESSAGE / MAIL / COURRIER.
  ...
  - **Liens BCDI auto + remboursement (juin 2026)** : les dossiers SAV manuels ont un champ `sellsyBdcRef` en texte libre (Michelle saisit des formats variés : `BCDI-05066 ET BCDI-05452`, `BCDEI-05109` typo, `BCDI-05083 / BC 11680`…). Résolution **à la volée** (pas de stockage) via `GET /api/sav/[id]/sellsy` : `parseBcdiRefs()` (`lib/sav-bcdi.ts`, tolère typo/séparateurs) extrait tous les BCDI → recherche chaque commande Sellsy (`/orders/search` par numéro) → affiche statut + lien dans le drawer.
    - **Remboursement** = un **avoir** (note de crédit). ⚠ Sellsy rattache l'avoir au **CLIENT** (individual/contact), **pas au BCDI/commande** (la commande ne liste que les acomptes FAPJ dans ses `relateds`). On scanne donc tous les avoirs (`/credit-notes/search`, ~162, cache mémoire 10 min — filtres Sellsy non fiables) et on matche par id client (collectés sur les commandes résolues + `sellsyContactId`). `status` de l'avoir : `spent` = soldé / `stored` = émis non soldé.
    - **Semi-auto (choix Laurence)** : le drawer affiche une bannière « Remboursement détecté — AVR-xxxx, soldé/non soldé » + bouton **« Marquer ce SAV traité »** (1 clic → statut TRAITE). Pas de bascule auto (l'avoir étant lié au client, risque de faux positif). Validé : BCDI-05245 (David CHANE) → AVR-2026-0163 soldé.
