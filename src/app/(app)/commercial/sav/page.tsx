...
function CreationModal({ onClose, onSuccess, users }: CreationModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    sellsyBdcRef: "",
    titre: "",
    type: TYPES_SAV[0].value as string,
    description: "",
    assigneAId: "",
  });
  ...
  const [contactResults, setContactResults] = useState<{ id: string; nom: string; prenom: string; email: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/contacts?search=${encodeURIComponent(contactSearch)}&limit=8`);
        if (res.ok) {
          const data = await res.json();
    ...
    try {
      const res = await fetch("/api/sav", {
        ...
        body: JSON.stringify({
          ...
          assigneId: form.assigneAId || null,
        }),
      });
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="relative">
              ...
              {form.contactId ? (
                <div className="flex items-center gap-2 bg-cockpit-dark border border-cockpit rounded-lg px-3 py-2.5">
                  <span className="text-sm font-medium text-cockpit-heading flex-1">{form.contactNom}</span>
                  <button
                    type="button"
                    onClick={() => {
                      updateField("contactId", "");
                      updateField("contactNom", "");
                      setContactSearch("");
                    }}
                    className="p-1 hover:bg-cockpit rounded text-cockpit-secondary"
                  >
                ...
                <>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-cockpit-dark border border-cockpit rounded-lg pl-9 pr-3 py-2.5 text-sm text-cockpit-primary placeholder:text-cockpit-secondary focus:outline-none focus:ring-2 focus:ring-teal-500/40"
                    />
                    {searchingContacts && (
                      <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 animate-spin text-cockpit-secondary" />
                    )}
                  </div>
                  {showContactDropdown && contactResults.length > 0 && (
                    <div className="absolute z-50 w-full mt-1 bg-white border border-cockpit rounded-lg shadow-lg max-h-48 overflow-y-auto">
                      {contactResults.map((c) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            updateField("contactNom", `${c.prenom} ${c.nom}`);
                            setContactSearch("");
                            setShowContactDropdown(false);
                          }}
                          className="w-full text-left px-3 py-2 hover:bg-cockpit-dark transition-colors text-sm"
                        >
                          <span className="font-medium text-cockpit-heading">{c.prenom} {c.nom}</span>
                          {c.email && <span className="ml-2 text-xs text-cockpit-secondary">{c.email}</span>}
                        </button>
                      ))}
                    </div>
                  )}
                  {showContactDropdown && contactResults.length === 0 && contactSearch.length >= 2 && !searchingContacts && (
                    <div className="absolute z-50 w-full mt-1 bg-white border border-cockpit rounded-lg shadow-lg p-3 text-xs text-cockpit-secondary text-center">
