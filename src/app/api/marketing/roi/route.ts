...
// --- Google Ads monthly spend — via le Google Sheet importé (table Campagne) ---
async function fetchGoogleMonthlySpend(annee: string): Promise<Record<string, number>> {
  // Source unique : table Campagne, alimentée par l'import du Google Sheet
  // (lui-même rempli par le script Google Ads). Plus d'appel à l'API Google Ads.
  try {
